...
Usage:
    python3 backfill/backfill_history.py                 # all of 2016-2024
    python3 backfill/backfill_history.py 2019 2020       # specific seasons
...
def backfill_team_stats(apis, y):
    ...
    for s in advanced:
        ...
        if not o and not d:
            continue
        # team_stats — 35 legacy columns + season (mirrors pipeline/fetch_team_stats.py)
        cursor.execute('''
        ...
        # team_advanced — 19 legacy columns + season (mirrors pipeline/fetch_advanced.py)
...
def backfill_ap(apis, y):
    ...
    same one-snapshot-per-season convention pipeline/fetch_rankings.py uses for 2025."""
