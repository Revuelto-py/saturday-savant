...
Same pipeline as backfill/setup_db.py / tools/upload_headshots_to_r2.py — ESPN CDN
...
Stages (each resumable — skip-existing at every step):
    ...
    2. nfl        NFL-CDN fallback for players ESPN purged from the college CDN but
                  ...
                  (the college->NFL id continuity backfill/backfill_nfl.py relies on).
...
Run:  python3 backfill/backfill_headshots.py            # all stages
      python3 backfill/backfill_headshots.py download   # a single stage
"""
...
def stage_upload():
    ...
    # Preflight (same as tools/upload_headshots_to_r2.py)
