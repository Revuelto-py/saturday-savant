...
# Nine steps below are deliberately non-fatal: a third-party hiccup must not
...
echo "── [1/20] player box scores + PPA (fetch_data) ──"
...
echo "── [2/20] label week 0 (apply_week_zero) ──"
...
echo "── [3/20] player game logs, active season (backfill_game_logs) ──"
$PY backfill/backfill_game_logs.py --current || note_fail "game-log refresh failed — logs rebuild lazily, continuing"
echo "── [4/20] team stats (fetch_team_stats) ──"
$PY pipeline/fetch_team_stats.py
echo "── [5/20] advanced team stats (fetch_advanced) ──"
$PY pipeline/fetch_advanced.py
echo "── [6/20] SP+ ratings (fetch_sp) ──"
$PY pipeline/fetch_sp.py
echo "── [7/20] AP rankings (fetch_rankings) ──"
$PY pipeline/fetch_rankings.py
echo "── [8/20] head coaches, current season (fetch_coaches) ──"
...
echo "── [9/20] team rosters, current season (fetch_2026_roster) ──"
...
echo "── [10/20] EA ratings, starter-model input (fetch_ea_ratings) ──"
...
echo "── [11/20] player headshots, current roster (refresh_headshots) ──"
...
echo "── [12/20] game summaries / drives (fetch_game_summaries) ──"
...
echo "── [13/20] play-level passing: air yards / location / YAC (fetch_passing) ──"
...
echo "── [14/20] enriched rushing: directions / line yards / explosiveness (fetch_rushing) ──"
$PY pipeline/fetch_rushing.py || note_fail "(rushing fetch failed — rushing splits keep last week's data)"
echo "── [15/20] Savant ratings (compute_savant_ratings) ──"
$PY pipeline/compute_savant_ratings.py --write   # --write persists; without it the script only dry-runs
echo "── [16/20] percentile peer pools (backfill_pools) ──"
$PY pipeline/backfill_pools.py
echo "── [17/20] team-page + returning-production precompute (precompute) ──"
$PY pipeline/precompute.py
echo "── [18/20] Vegas lines, active season (fetch_betting_lines) ──"
$PY pipeline/fetch_betting_lines.py
echo "── [19/20] Savant Forecast: score last week + predict upcoming (predict_games) ──"
...
echo "── [20/20] label week 0 in the tables written since (apply_week_zero) ──"
$PY pipeline/apply_week_zero.py || note_fail "week 0 pass failed — weeks unchanged, continuing"
