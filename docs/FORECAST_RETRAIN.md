...
- **FBS vs FBS** — `forecast_model.json` (16 features), trained by
  `forecast/train_forecast.py`. The main model; everything above refers to it.
- **FBS vs FCS** — `fcs_forecast_model.json`, trained by `forecast/train_fcs_forecast.py`
  ...
  hash stays `9615cb34…`, verified). pipeline/predict_games.py loads both artifacts and
...
2. **Re-backfill stored `contrib` rows.** Every scored prediction stores the
   ...
   ```bash
   python3 backfill/backfill_forecast_contrib.py          # dry run: reports match rate
   python3 backfill/backfill_forecast_contrib.py --write
   ```
...
Upcoming-game rows need nothing: `pipeline/predict_games.py` writes `contrib` on every
...
`tools/fetch_forecast_extras.py` (venues / weather / talent / coaches / opening
lines), `tools/build_game_boxstats.py` (turnovers, penalties, possession from the
stored ESPN summaries), `forecast/forecast_candidates.py` (feature builders + the
walk-forward/McNemar harness). Re-run those three, then
`python3 forecast/forecast_candidates.py`, to repeat the sweep with a new season added.
...
into `forecast/forecast_candidates.py` and run under the standing protocol. Baseline
...
`forecast/forecast_candidates.py` (`INVESTIGATION_2026_07`) for the January re-check —
...
- **2026+ (live):** written by `pipeline/predict_games.py` before kickoff, frozen on
  scoring. The real going-forward record.
- **2025 (backfilled backtest):** written once by `backfill/backfill_2025_forecasts.py`.
...
Frozen-safety: `pipeline/predict_games.py` only touches `season = current_cfb_season()`
