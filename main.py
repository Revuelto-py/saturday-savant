...
PERCENTILE_METRICS = {
    ...
    'RB': [
        ...
        # and are None until 20 of his carries are actually enriched, so a back
        # the source has barely measured shows his other bars and none of these.
        # Stuff rate is the one bar where lower is better.
        ('Success Rate',      'rush',      'sr',           True),
        ('Line Yards',        'rush',      'line_yds',     True),
        ('Second-Level Yards','rush',      'sec_yds',      True),
...
def _build_percentiles(cursor, player_id, pos, season=CURRENT_SEASON):
    ...
    for src in needed:
        if src in ('passing', 'rushing', 'receiving', 'defensive',
            ...
            # Same INTEGER-vs-TEXT player_id trap as 'air' above: without str()
            # every lookup misses and the rushing bars silently vanish.
            raw = get_season_rusher_metrics(season)
            sources['rush'] = ({str(pid): d for pid, d in raw.items()
                                if str(pid) in qualified}, False)
        elif src == 'ints':
...
PLAYER_COLUMNS = {
    ...
    'rushing': {
        ...
        'advanced': [
            ...
            # From the enriched rushing feed. Every one of these is blank until
            # 20 of a back's carries are measured, and the count they are
            # measured over is the RC column beside them — not his carry total.
            ('rush_att',  'RC',    'Rushes with enriched data behind them — the sample every column to the right is measured over, which can differ from his carry total', True, 'int'),
            ('sr',        'SR%',   'Success Rate — share of his runs that kept the offense on schedule', True, 'pct1'),
            ('line_yds',  'LINE',  'Line Yards per carry — the part of a run credited to the blocking in front of him', True, 'float1'),
            ('sec_yds',   '2ND',   'Second-Level Yards per carry — yards gained 5 to 10 past the line, where the back beats the linebackers', True, 'float1'),
            ('open_yds',  'OPEN',  'Open-Field Yards per carry — everything past 10 yards, which is the back running away from the defense', True, 'float1'),
            ('stuff',     'STF%',  'Stuffed Rate — share of carries stopped at or behind the line. Lower is better, so the heat is inverted', True, 'pct1'),
            ('power',     'POW%',  'Power Success — short-yardage conversions on third or fourth and 2 or less', True, 'pct1'),
            ('expl',      'EXPL',  'Explosiveness — average EPA of his successful runs, so a back who breaks long ones rates above one who grinds', True, 'float1'),
        ],
    },
    'receiving': {
        'standard': [
...
def leaderboards(category='passing'):
    ...
    try:
        ...
        if category == 'passing':
            ...
            # Enriched rushing for the whole season in one read, the same way the
            # passing board pulls air yards. Keyed on the TEXT player id, so the
            # lookup below stringifies p.id — the passing map keys on INTEGER.
            _rusher = get_season_rusher_metrics(season) if view == 'advanced' else {}
            ...
            for r in cursor.fetchall():
                ...
                if view == 'advanced':
                    ...
                    row['usage'] = usage_val
                    # A back the feed has not measured, and one under the
                    # 20-carry floor, both blank out rather than showing a rate
                    # drawn from a handful of runs.
                    row.update(_rusher.get(str(r[0])) or {})
                    for _k in ('rush_att', 'sr', 'line_yds', 'sec_yds',
                               'open_yds', 'stuff', 'power', 'expl'):
...
def game_detail(game_id):
    ...
    if is_scheduled:
        ...
        return render_template('game.html',
            ...
            is_scheduled=True, is_live=is_live, game_passing=None, advanced=None,
    ...
    return render_template('game.html',
        ...
        advanced=get_game_advanced(game_id, home_team, away_team),
        home_team=home_team,
        away_team=away_team,
...
# ── Enriched rushing (pipeline/fetch_rushing.py) ────────────────────────────
# Success rate, line / second-level / open-field yards, stuff rate, power
# success and explosiveness, split left / middle / right.
#
# `attempts` on these rows counts ENRICHED attempts, not carries: it is the
# denominator for every rate beside it, it can differ from a player's carry
# total, and coverage varies by game. So everything below gates on it, and the
...
def get_season_rusher_metrics(season):
    ...
    Values are None below MIN_RUSH_ATTEMPTS enriched attempts: a stuff rate off
    six carries is noise wearing a season label.
    """
    conn = get_db()
    try:
        cur = conn.cursor()
        cur.execute("""
            SELECT player_id, attempts, ypc, success_rate, ppa,
        ...
        """, (season,))
        out = {}
        for (pid, att, ypc, sr, ppa, line, second, openf, stuff, power, expl) in cur.fetchall():
            ok = (att or 0) >= MIN_RUSH_ATTEMPTS
            out[str(pid)] = {
                'rush_att': int(att or 0),
                'rush_ypc': round(ypc, 1) if (ok and ypc is not None) else None,
                'sr': round(sr * 100, 1) if (ok and sr is not None) else None,
                'rush_ppa': round(ppa, 3) if (ok and ppa is not None) else None,
                'line_yds': round(line, 1) if (ok and line is not None) else None,
                'sec_yds': round(second, 1) if (ok and second is not None) else None,
                'open_yds': round(openf, 1) if (ok and openf is not None) else None,
                'stuff': round(stuff * 100, 1) if (ok and stuff is not None) else None,
                'power': round(power * 100, 1) if (ok and power is not None) else None,
                'expl': round(expl, 2) if (ok and expl is not None) else None,
            }
        return out
    except Exception:
...
# ── Advanced box score ───────────────────────────────────────────────────────
# CFBD's per-game advanced box: EPA and success rate by quarter, havoc, scoring
# opportunities, field position, the rushing line-yard splits, and every
# player's usage and EPA. Stored on first view exactly like the ESPN summary —
# a finished game's advanced box never changes, so one visitor pays the upstream
# call and nobody after them does. No cron warms it: a game nobody opens costs
# nothing, and the table only grows with what readers actually look at.

def _adv_val(d, *names):
    """First present key. The CFBD client has shipped both camelCase and
    snake_case shapes for these payloads; reading both means a client upgrade
    cannot quietly empty the tab."""
    for n in names:
...
def _adv_quarters(d):
    """{'game': v, 'q1': v, …} from one CFBD split block."""
    if not isinstance(d, dict):
        return {}
    out = {'game': _adv_val(d, 'total')}
    for i in (1, 2, 3, 4):
        out[f'q{i}'] = _adv_val(d, f'quarter{i}', f'quarter_{i}')
    return out


def _adv_by_team(rows):
    return {r.get('team'): r for r in (rows or []) if isinstance(r, dict) and r.get('team')}


def _adv_fetch(game_id):
    """One CFBD call. Imported here rather than at module scope: the web app
    has no other use for the SDK and a worker should not pay its import.

    Bounded, because this runs inside a request. An upstream that hangs would
    otherwise hold the worker until gunicorn kills it — and a killed worker is
    what poisons the connection pool for everyone else. Ten seconds is far more
    than the call needs; past that the tab is simply absent for this visitor and
    the next one tries again."""
    import cfbd
    cfg = cfbd.Configuration(access_token=os.getenv('CFBD_API_KEY'))
    with cfbd.ApiClient(cfg) as api:
        res = cfbd.GamesApi(api).get_advanced_box_score(
            id=game_id, _request_timeout=(4, 10))
    return res.to_dict() if hasattr(res, 'to_dict') else res


def _adv_raw(game_id):
    """The stored payload for a game, fetching and storing it on a miss.
    Returns None when CFBD has nothing — a game outside play-by-play coverage,
    which is a real answer, not an error."""
    conn = get_db()
    try:
        cur = conn.cursor()
        cur.execute('SELECT payload_gz FROM game_advanced WHERE game_id = %s', (game_id,))
        row = cur.fetchone()
        if row and row[0]:
            return json.loads(gzip.decompress(bytes(row[0])))
    except Exception:
        conn.rollback()      # table not created yet — fall through to the fetch
    finally:
        release_db(conn)

    try:
        raw = _adv_fetch(game_id)
    except Exception as e:
        print(f'advanced box fetch failed for {game_id}: {e}')
        return None
    # Store only a payload that carries the team blocks. CFBD answers for a game
    # it has not processed yet with the envelope and nothing in it, and a thin
    # blob stored once would be served forever — the same trap the ESPN summary
    # fell into when it cached a header-only response minutes after a final.
    if not raw or not (raw.get('teams') or {}).get('ppa'):
        return None
    connS = get_db()
    try:
        curS = connS.cursor()
        ...
        ''')
        curS.execute('''
            INSERT INTO game_advanced (game_id, payload_gz) VALUES (%s, %s)
            ON CONFLICT (game_id) DO UPDATE SET payload_gz = EXCLUDED.payload_gz,
                                                updated_at = now()
        ''', (game_id, gzip.compress(json.dumps(raw, default=str).encode())))
        connS.commit()
    except Exception:
        connS.rollback()
    finally:
...
def _adv_fmt(v, kind):
    """Display string for one metric. A dash is "not supplied"; zero is a
    measured value and prints as one."""
    if v is None:
        return '—'
    if kind == 'pct':
        return f'{v * 100:.1f}%'
...
@cache.memoize(timeout=21600)
def get_game_advanced(game_id, home_team, away_team):
    """Render-ready advanced box score, or None.

    Every figure is CFBD's own; nothing here is derived, so a metric CFBD does
    not supply for a game arrives as None and prints as a dash rather than a
    zero it never measured.
    """
    raw = _adv_raw(game_id)
    if not raw:
        return None
    teams = raw.get('teams') or {}
    ppa   = _adv_by_team(teams.get('ppa'))
    cum   = _adv_by_team(_adv_val(teams, 'cumulativePpa', 'cumulative_ppa'))
    succ  = _adv_by_team(_adv_val(teams, 'successRates', 'success_rates'))
    expl  = _adv_by_team(teams.get('explosiveness'))
    rush  = _adv_by_team(teams.get('rushing'))
    havoc = _adv_by_team(teams.get('havoc'))
    opps  = _adv_by_team(_adv_val(teams, 'scoringOpportunities', 'scoring_opportunities'))
    fpos  = _adv_by_team(_adv_val(teams, 'fieldPosition', 'field_position'))
    if not ppa:
        return None

    sides = {'away': away_team, 'home': home_team}

    def per(getter):
        """{'away': v, 'home': v} from a team-keyed lookup."""
        return {s: getter(sides[s]) for s in ('away', 'home')}

    def q_of(src, team, block):
        return _adv_quarters((src.get(team) or {}).get(block) or {})

    # ── The comparison rows ──────────────────────────────────────────────
    # `higher` says which direction is better, and drives the one mark this tab
    # makes: the better of the two numbers is the brighter one. False inverts
    # it — average field start is yards TO the goal, so lower is better field
    # position, and a stuffed run is the defence's win.
    #
    # None means the row is not a contest and NEITHER side is marked. Snap count
    # and average depth of target are descriptions of how a team played, not
    # scores, and marking one of them "better" would quietly assert something
    # the number does not say.
    def row(label, values, kind, higher=True, note=''):
        vals = {s: values.get(s) for s in ('away', 'home')}
        best = None
    ...
    rushing = [
        ...
        row('Power success',   per(lambda t: _adv_val(rush.get(t, {}), 'powerSuccess', 'power_success')), 'pct'),
        row('Stuff rate',      per(lambda t: _adv_val(rush.get(t, {}), 'stuffRate', 'stuff_rate')), 'pct', higher=False),
        row('Line yards / rush',
            per(lambda t: _adv_val(rush.get(t, {}), 'lineYardsAverage', 'line_yards_average')), 'f2'),
        row('Second-level yards / rush',
            per(lambda t: _adv_val(rush.get(t, {}), 'secondLevelYardsAverage', 'second_level_yards_average')), 'f2'),
        row('Open-field yards / rush',
            per(lambda t: _adv_val(rush.get(t, {}), 'openFieldYardsAverage', 'open_field_yards_average')), 'f2'),
    ]
    ...
    def splits(team):
        return [
            {'label': 'Average EPA',    'kind': 'epa', 'q': q_of(ppa, team, 'overall')},
            {'label': 'Passing EPA',    'kind': 'epa', 'q': q_of(ppa, team, 'passing')},
            {'label': 'Rushing EPA',    'kind': 'epa', 'q': q_of(ppa, team, 'rushing')},
            {'label': 'Total EPA',      'kind': 'sum', 'q': q_of(cum, team, 'overall')},
            {'label': 'Passing total',  'kind': 'sum', 'q': q_of(cum, team, 'passing')},
            {'label': 'Rushing total',  'kind': 'sum', 'q': q_of(cum, team, 'rushing')},
            {'label': 'Success rate',   'kind': 'pct', 'q': q_of(succ, team, 'overall')},
            {'label': 'Standard downs', 'kind': 'pct', 'q': q_of(succ, team, _std_key(succ, team))},
            {'label': 'Passing downs',  'kind': 'pct', 'q': q_of(succ, team, _pd_key(succ, team))},
            {'label': 'Explosiveness',  'kind': 'f2',  'q': q_of(expl, team, 'overall')},
        ]

    def split_table(team):
        rows = []
        for r in splits(team):
            cells = [{'k': k, 'd': _adv_fmt(r['q'].get(k), r['kind'])}
                     for k in ('game', 'q1', 'q2', 'q3', 'q4')]
            rows.append({'label': r['label'], 'cells': cells})
        return rows

    # ── Players: usage and EPA, merged on name + team ────────────────────
    players = []
    pl = raw.get('players') or {}
    usage_by = {(u.get('player'), u.get('team')): u
                for u in (pl.get('usage') or []) if isinstance(u, dict)}
    for p in (pl.get('ppa') or []):
        if not isinstance(p, dict):
            continue
        avg  = p.get('average') or {}
        tot  = p.get('cumulative') or {}
        u    = usage_by.get((p.get('player'), p.get('team')), {})
        players.append({
            'name': p.get('player'), 'team': p.get('team'), 'pos': p.get('position'),
            'is_home': p.get('team') == home_team,
            'usage':      u.get('total'), 'usage_d':   _adv_fmt(u.get('total'), 'pct'),
            'pass_usage': u.get('passing'), 'pass_d':   _adv_fmt(u.get('passing'), 'pct'),
            'rush_usage': u.get('rushing'), 'rush_d':   _adv_fmt(u.get('rushing'), 'pct'),
            'avg':  avg.get('total'), 'avg_d':   _adv_fmt(avg.get('total'), 'epa'),
            'total': tot.get('total'), 'total_d': _adv_fmt(tot.get('total'), 'sum'),
        })
    # Most EPA created first — the reader's question is who moved the game.
    players.sort(key=lambda r: (r['total'] is None, -(r['total'] or 0)))

    # ── Passing, merged with the play-level table ────────────────────────
    # CFBD's advanced box carries the passing EPA; passing_plays carries the
    # shape of the attempts (2025 on). Both describe the same dropbacks, so
    # they belong in one block rather than two the reader has to join.
    pp = get_game_passing_profile(game_id) or {}

    def pv(getter):
        return {s: getter(pp.get(sides[s]) or {}) for s in ('away', 'home')}

    def _rate(d):
        a, c = d.get('attempts'), d.get('completions')
        return (c / a) if a else None

    def _ypa(d):
        a, y = d.get('attempts'), d.get('yards')
        return (y / a) if a else None

    passing = [
        row('EPA / pass play',
            per(lambda t: (ppa.get(t, {}).get('passing') or {}).get('total')), 'epa'),
        row('Total passing EPA',
            per(lambda t: (cum.get(t, {}).get('passing') or {}).get('total')), 'sum'),
    ]
    if pp:
        passing += [
            row('Completion rate',    pv(_rate), 'pct'),
            row('Yards / attempt',    pv(_ypa), 'f2'),
            row('Average depth of target', pv(lambda d: d.get('adot')), 'f1',
                higher=None, note='how far downfield, not better or worse'),
            row('Yards after catch / reception',
                pv(lambda d: d.get('yac_avg')), 'f1'),
        ]

    info = raw.get('gameInfo') or raw.get('game_info') or {}
    wp = {'home': _adv_val(info, 'homeWinProb', 'home_win_prob'),
          'away': _adv_val(info, 'awayWinProb', 'away_win_prob')}
    return {
...
def _std_key(src, team):
    """CFBD names the standard-down block standardDowns; older client builds
    emit standard_downs. Resolve once per lookup rather than guessing."""
    d = src.get(team) or {}
    return 'standardDowns' if 'standardDowns' in d else 'standard_downs'


def _pd_key(src, team):
    d = src.get(team) or {}
    return 'passingDowns' if 'passingDowns' in d else 'passing_downs'
