...
predict_games.py writes `contrib` alongside every new prediction, but scored
rows are frozen — it only ever updates `WHERE scored = 0` — so the 2025 rows
laid down by backfill_2025_forecasts.py have a probability and no breakdown.
...
Usage:  python3 backfill_forecast_contrib.py            # dry run: reports match rate
        python3 backfill_forecast_contrib.py --write    # persist contrib
"""
import json
import math
import os
